...
const ShareWorkspaceModal = ({
  ...
  return (
    ...
    >
      <div
        className="modal w-full max-w-2xl bg-gray-900/80 backdrop-blur-xl border border-gray-700/60 rounded-2xl p-6 md:p-8 shadow-2xl flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-header flex items-center justify-between mb-6 shrink-0">
          <h2 className="modal-title text-xl md:text-2xl font-semibold font-display tracking-tight text-white pr-4 truncate">
            Share "{workspaceName}"
          </h2>
          <button
            className="modal-close text-gray-400 hover:text-white hover:bg-white/10 p-1.5 rounded-lg transition"
            onClick={onClose}
            aria-label="Close"
          >
            <X size={18} />
        ...
        <form
          ...
          className="modal-form flex flex-col gap-4 mb-8 shrink-0"
        >
          <div className="share-row flex flex-col sm:flex-row gap-4 items-start sm:items-end">
            ...
            <div className="form-group w-full sm:w-auto mt-2 sm:mt-0">
              <button
                type="submit"
                className="btn btn-primary h-12 px-6 w-full min-w-[100px]"
                disabled={loading}
              >
                {loading ? (
        ...
        <div className="share-link-section border-t border-gray-800/60 pt-4 mt-4">
          <h3 className="text-sm font-semibold tracking-wider text-gray-500 uppercase mb-3 flex items-center gap-1.5">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              className="btn btn-primary h-10 px-4 text-sm whitespace-nowrap flex items-center gap-1.5"
              onClick={async () => {
                try {
          ...
          <div className="member-list flex-1 overflow-y-auto custom-scrollbar pr-2 border-t border-gray-800/60 pt-6">
            ...
            <div className="flex flex-col gap-2">
              {members.map((m) => (
                <div
                  key={m._id}
                  className="member-row flex items-center justify-between p-3 bg-gray-800/20 hover:bg-gray-800/40 border border-gray-800/60 rounded-xl transition-colors"
                >
                  <div className="member-info flex items-center gap-3">
                    ...
                    <div>
                      <p className="member-name text-sm font-medium text-gray-200">
                        {m.userId?.username}
                      </p>
                      <p className="member-email text-xs text-gray-500">
                        {m.userId?.email}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`role-badge px-3 py-1 rounded-full text-xs font-semibold tracking-wide uppercase ${m.role === "owner" ? "bg-violet-500/15 text-violet-400 border border-violet-500/30" : m.role === "editor" ? "bg-blue-500/15 text-blue-400 border border-blue-500/30" : "bg-gray-500/15 text-gray-400 border border-gray-500/30"}`}
                  >
                    {m.role}
                  </span>
                </div>
