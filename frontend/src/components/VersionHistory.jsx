  ...
  return (
    <div className="version-history bg-gray-900/40 backdrop-blur-xl border border-gray-700/60 rounded-xl overflow-hidden flex flex-col h-full max-h-[400px]">
      <div className="history-header flex items-center justify-between px-4 py-3 border-b border-gray-700/60 bg-gray-800/40">
        <span className="history-title text-sm font-semibold tracking-wider text-gray-400 uppercase flex items-center gap-1.5">
          <History size={14} /> History
        </span>
        {canEdit && (
          <button
            className="btn-save-snapshot text-xs font-medium bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 border border-blue-500/20"
      ...
      <div className="history-list flex-1 overflow-y-auto p-3 custom-scrollbar">
        {loading ? (
          <div className="history-loading text-sm text-gray-500 text-center py-4">
            Loading history...
          </div>
        ) : history.length === 0 ? (
          <div className="history-empty text-sm text-gray-500 text-center py-4 italic">
            No versions found.
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {history.map((v) => (
              <div
                key={v._id}
                className="history-item flex items-center justify-between p-3 bg-gray-800/30 hover:bg-gray-800/60 border border-gray-700/40 rounded-lg transition-colors group"
              >
                <div className="history-item-info flex flex-col">
                  <span className="history-date text-sm font-medium text-gray-300">
                    {new Date(v.createdAt).toLocaleString()}
                  </span>
                  <span className="history-user text-xs text-gray-500">
                    by{" "}
