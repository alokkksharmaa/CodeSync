...
const VoiceChat = ({ socket, workspaceId, userId, username }) => {
  ...
  const handleIceCandidate = async (fromUserId, candidate) => {
    try {
      ...
      if (peer) {
        ...
        console.warn(
          "[VoiceChat] No peer connection found for ICE candidate from:",
          fromUserId,
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleUserJoined = async ({
      ...
      if (localStreamRef.current && joinedUserId !== userId) {
        console.log(
          "[VoiceChat] Creating offer for new participant:",
          joinedUserId,
        );
        try {
          ...
          console.error(
            "[VoiceChat] Error creating offer for new participant:",
            err,
          );
        }
      }
    ...
    const handleVoiceAnswer = async ({ from, answer }) => {
      console.log(
        "[VoiceChat] Received answer from:",
        from,
        "my userId:",
        userId,
      );
    ...
    const handleVoiceIceCandidate = async ({ from, candidate }) => {
      console.log(
        "[VoiceChat] Received ICE candidate from:",
        from,
        "my userId:",
        userId,
      );
  ...
  const startVoiceChat = async () => {
    try {
      ...
      console.log(
        "[VoiceChat] Got local stream with",
      ...
      participants.forEach(async (participant) => {
        if (participant.userId !== userId) {
          console.log(
            "[VoiceChat] Creating peer connection for existing participant:",
            participant.userId,
          );
          ...
          console.log(
            "[VoiceChat] Sending offer to existing participant:",
            participant.userId,
          );
          socket.emit("voice_offer", {
